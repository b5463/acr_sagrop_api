...
router.post("/upload-image", handleImageUpload, async (req, res) => {
    try {
        ...
        // Check the file type (example using the 'file-type' package)
        ...
        // Limit file size (adjust the limit as needed)
        ...
        // Define the directory where images will be stored securely
        const uploadDirectory = path.join(__dirname, "secure-uploads");
        ...
        // Generate the secure file path
        const imagePath = path.join(uploadDirectory, sanitizedFilename);

        // Write the file to the secure directory
        fs.writeFileSync(imagePath, imageBuffer);

        // Log the successful image upload
        logData("routes.js", `Image uploaded: ${imagePath}`, "info");

        // Respond with the URL of the uploaded image
        res.status(200).json({ imageUrl: `${req.protocol}://${req.get("host")}/images/${sanitizedFilename}` });
    } catch (error) {
